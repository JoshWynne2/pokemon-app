            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
					...
					<form action="{{route('teams.store')}}" method="POST">
						...
						<input type='text' name='name' id='name' class='block appearance-none w-1/2 bg-slate-700 border border-slate-700 hover:border-slate-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline'>
						<br>
						<label>Team Description</label>
						<input type='text' name='description' id='description' class='block appearance-none w-1/2 bg-slate-700 border border-slate-700 hover:border-slate-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline'>
						<br>
						<div>
							<label>Pokemon<label>
							<div class="grid grid-cols-3 gap-2">
								<select name="pokemon1" id="pokemon1" class="block appearance-none bg-slate-700 border border-slate-700 hover:border-slate-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
									...
									<option value="{{$mon->id}}"> {{$mon->name}} - ({{$mon->rname}}) </option>
									@empty
